...
:deep(.n-tree-node-content:active) {
  cursor: grabbing;
}
...
/* 拖拽到节点内部 */
:deep(.n-tree-node--drop-position-inside) {
  background: rgba(var(--n-info-color-rgb), 0.15);
...
/* 拖拽位置指示器 */
:deep(.n-tree-node--drop-position-before::before) {
  content: '';
  position: absolute;
  top: -2px;
  left: 0;
...
:deep(.n-tree-node--drop-position-after::after) {
  content: '';
  position: absolute;
  bottom: -2px;
  left: 0;
...
/* 深色模式适配 */
:global([data-theme="dark"]) :deep(.n-tree-node-content:hover) {
  background: rgba(255, 255, 255, 0.1);
}

:global([data-theme="light"]) :deep(.n-tree-node-content:hover) {
